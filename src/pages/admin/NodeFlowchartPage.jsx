...
import PhotoFlowNode from "../../components/PhotoFlowNode";
...
// Promoted from a modal (opened via a toolbar button on Node Editor) to
// its own Virtual Map sidebar destination — a visual flowchart of how
...
export default function NodeFlowchartPage() {
  ...
  // track live position updates while a node is being dragged.
  const [flowNodes, setFlowNodes, onNodesChange] = useNodesState([]);
  const [flowEdges, setFlowEdges] = useState([]);

  // Rebuilds ONLY when the scope itself changes (building/floor) — NOT
  // when `nodes` updates for any other reason. `nodes` is deliberately
  // read from closure here rather than listed as a dependency — see
  // NodeEditorPage's git history for the full reasoning (saving a dragged
  // position would otherwise re-trigger a full rebuild and reset the
  // viewport on every single drag). Live dragging itself is handled
  // entirely by onNodesChange/onNodeDragStop below, without this effect
  // re-running at all.
  useEffect(() => {
  ...
  return (
    <div className="node-editor-page">
      <h2 className="admin-page-heading">Node Flowchart</h2>
